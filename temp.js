        ...
        async function saveAndExit() {
            const targetFile = "Gestión de Grupos.html";
        ...
        function loadSelectedGroup() {
            ...
            if (!saved) {
                console.warn("⚠️ No se encontraron datos en 'selectedGroup'");
            ...
            try {
                ...
                // Detectar hotel desde Hotel_Asignado o Hotel
                const hotelRaw = (group["Hotel_Asignado"] || group["Hotel"] || "").toLowerCase();
                let detectedHotel = 'guadiana'; // Default
                if (hotelRaw.includes('cumb')) {
                    detectedHotel = 'cumbria';
                }
                console.log(`🏨 Hotel detectado: "${hotelRaw}" → plantilla: ${detectedHotel}`);
                ...
                const placeholders = ["Calle Dirección", "RAZÓN SOCIAL", "CP, Ciudad", "B00000000", "---", "70", "21-04-2026", "24-04-2026", "MARCELLO RONCHI", "marcello.ronchi@eapn.es", "EAPN-ES"];
                ...
                Object.keys(mapping).forEach(k => {
                    ...
                    const hasCustomData = customFields[k] && customFields[k] !== undefined && String(customFields[k]).trim() !== '' && !placeholders.includes(customFields[k]);
                    ...
                    if (isCritical) {
                        ...
                        if (hasFichaData) {
                            ...
                            // Si la ficha está vacía pero el usuario escribió algo manual (que no es placeholder), lo aceptamos
                            mapping[k] = customFields[k];
                        } else {
                            // Si ambos están vacíos o tienen placeholders, vaciamos el campo por completo
                            mapping[k] = "";
                        }
                    } else if (hasCustomData) {
                        // Para campos no críticos, permitimos el override normal
                        mapping[k] = customFields[k];
                    } else if (!hasFichaData) {
                        // Si no hay dato manual válido ni dato en la ficha, vaciamos para no mostrar placeholders feos (salvo en ciertos inputs)
                        if (['reserva-personas', 'reserva-entrada', 'reserva-salida', 'f-pax-count', 'f-date-in', 'f-date-out'].includes(k)) {
                            mapping[k] = "---";
                        } else {
                            mapping[k] = "";
                        }
                        ...
                        customFields.dynamicRows.forEach(row => {
                            ...
                            newRow.innerHTML = `
                                ...
                                <button class="no-print text-red-200 hover:text-red-500 text-[10px] ml-auto"
                                        onclick="this.parentElement.remove()" title="Eliminar">✕</button>
                            `;
                ...
                if (group.ProformaItems && Array.isArray(group.ProformaItems) && group.ProformaItems.length > 0) {
                    ...
                    if (importe > 0) {
                        items = [{
                            ...
                            precio: importe,
                    iva: 10,
                            regimen: group["Régimen"] || 'AD'
                        }];
                    }
                }
            } catch (e) {
                console.error("❌ Error en loadSelectedGroup:", e);
            }
        }
        ...
        window.addEventListener('DOMContentLoaded', async () => {
            ...
            loadSelectedGroup();
            renderItems();
            updatePaymentLink(); // Fuerza el href correcto desde el inicio
        });
        ...
        function switchTemplate(tpl) {
            ...
            document.getElementById('tpl-propuesta').classList.toggle('hidden', tpl !== 'propuesta');
            ...
            document.getElementById('btn-propuesta').className = tpl === 'propuesta' ? 'pb-2 px-2 tab-active transition-all uppercase text-xs tracking-widest' : 'pb-2 px-2 text-gray-400 transition-all uppercase text-xs tracking-widest';

            const docTitle = document.getElementById('doc-type-title');
            const docLabel = document.getElementById('doc-type-label');
            if (docTitle) docTitle.innerText = tpl === 'confirmacion' ? 'Confirmación' : 'Proforma';
            if (docLabel) docLabel.innerText = tpl === 'confirmacion' ? 'Reserva:' : 'Nº Factura:';
        ...
        function formatNum(num) {
            ...
            // Usa toLocaleString para forzar formato español: 1.234,56
        ...
        function renderItems() {
                    ...
                    const row = `
                        <tr class="border-b group" ${draggables}>
                            <td class="p-1 flex items-center gap-1">
                                <span class="no-print opacity-0 group-hover:opacity-40 cursor-grab">⋮⋮</span>
            ...
            if (currentTemplate === 'propuesta') {
                const dailyC = groupData.dailyConfig || {};
                const dates = Object.keys(dailyC).sort();
                const container = document.querySelector('.propuesta-items');
                container.innerHTML = '';

                // 1. Mostrar Desglose por Noche
                dates.forEach((date, idx) => {
                    const rooms = dailyC[date];
                    const roomEntries = Object.entries(rooms);
                    let dailyTotal = 0;
                    let dailyRegimen = "";

                    const roomHtml = roomEntries.map(([type, cfg]) => {
                        const count = (groupData.roomCounts || {})[type] || 0;
                        const sub = count * (parseFloat(cfg.price) || 0);
                        dailyTotal += sub;
                        dailyRegimen = cfg.board || dailyRegimen;
                        return `<div class="flex justify-between items-center py-1">
                                    <span class="font-bold text-slate-600">${count}x ${type} <span class="text-[8px] font-normal text-slate-400">(a ${formatNum(cfg.price)}€)</span></span>
                                    <span class="font-black text-slate-800">${formatNum(sub)}€</span>
                                </div>`;
                    }).join('');
                    ...
                    const row = `
                        <tr class="bg-white">
                            <td class="p-6 align-top">
                                <span class="bg-indigo-50 text-indigo-700 px-3 py-1 rounded-lg text-xs font-black uppercase tracking-widest">${formatDate(date)}</span>
                                <p class="text-[9px] font-bold text-slate-400 uppercase mt-2">Noche ${idx + 1}</p>
                            </td>
                            <td class="p-6 text-center align-middle">
                                <span class="px-3 py-1 bg-slate-100 rounded-full text-[9px] font-black text-slate-600 uppercase tracking-widest">${dailyRegimen}</span>
                            </td>
                            <td class="p-6 align-top">
                                <div class="space-y-1">
                                    ${roomHtml}
                            ...
                            <td class="p-6 text-right align-middle">
                                <span class="text-sm font-black text-[#2d5a43]">${formatNum(dailyTotal)}€</span>
                            </td>
                        </tr>
                    `;
                ...
                // 2. Mostrar Otros Conceptos si existen
                if (items.length > 0) {
                    const otherItemsHeader = `
                        <tr class="bg-slate-50 border-y border-slate-200">
                            <td colspan="4" class="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">Otros Servicios y Conceptos Adicionales</td>
                        </tr>
                    `;
                    container.insertAdjacentHTML('beforeend', otherItemsHeader);

                    items.forEach((item, idx) => {
                        const sub = (parseFloat(item.precio) || 0) * (parseFloat(item.hab) || 1) * (parseFloat(item.cant) || 1) * (parseFloat(item.dias) || 1);
                        const row = `
                            <tr class="bg-white/50">
                                <td class="p-6 text-[10px] font-bold text-slate-400 uppercase">${formatDate(item.fecha)}</td>
                                <td class="p-6 text-center">
                                    <span class="px-3 py-1 bg-amber-50 rounded-full text-[9px] font-black text-amber-600 uppercase tracking-widest">${item.regimen || '-'}</span>
                                </td>
                                <td class="p-6">
                                    <div class="flex justify-between items-center">
                                        <span class="font-black text-slate-700 uppercase">${item.concepto}</span>
                                        <span class="text-[9px] font-bold text-slate-400">UNID: ${item.cant}</span>
                                    </div>
                                </td>
                                <td class="p-6 text-right">
                                    <span class="text-sm font-black text-slate-800">${formatNum(sub)}€</span>
                                </td>
                            </tr>
                        `;
                        container.insertAdjacentHTML('beforeend', row);
                ...
                document.querySelectorAll('#programa-3').forEach(el => el.innerText = document.getElementById('programa-1')?.innerText || groupData["Nombre del Grupo"] || '---');
                document.querySelectorAll('#solicitante-org-3').forEach(el => el.innerText = document.getElementById('solicitante-org-1')?.innerText || groupData["Empresa/Agencia"] || '---');
                document.querySelectorAll('#reserva-entrada-3').forEach(el => el.innerText = document.getElementById('reserva-entrada-1')?.innerText || groupData["Entrada"] || '---');
                document.querySelectorAll('#reserva-salida-3').forEach(el => el.innerText = document.getElementById('reserva-salida-1')?.innerText || groupData["Salida"] || '---');
                document.querySelectorAll('#solicitante-nombre-3').forEach(el => el.innerText = document.getElementById('solicitante-nombre-1')?.innerText || groupData["Com_Nombre_Contacto"] || '---');
                document.querySelectorAll('#solicitante-email-3').forEach(el => el.innerText = document.getElementById('solicitante-email-1')?.innerText || groupData["Com_Email_Contacto"] || '---');
            }
            ...
            if (currentTemplate === 'confirmacion') {
                ...
                // Siempre recalcular importes basados en el neto actual para evitar discrepancias
                if (groupPlan.length > 0) {
                    const pendingRows = groupPlan.filter(p => p.status !== "Cobrado");
                    ...
                    if (pendingRows.length > 0) {
                        ...
                        pendingRows.forEach((p, idx) => {
                            if (pendingRows.length === 1) {
                                p.amount = totalToDistribute.toFixed(2);
                            } else {
                                const weight = (parseFloat(p.percent) || 0) / totalPendingPercent;
                                p.amount = (totalToDistribute * weight).toFixed(2);
                            }
                        });
                    }
                }
                ...
                if (groupPlan.length === 0) {
                    ...
                    groupPlan = [
                        { label: "1er Pago Garantía", percent: 30, amount: (totalNetoPagar * 0.3).toFixed(2), date: d1.toISOString().split('T')[0], status: "Pendiente" },
                        { label: "Liquidación Final", percent: 70, amount: (totalNetoPagar * 0.7).toFixed(2), date: d2.toISOString().split('T')[0], status: "Pendiente" }
                    ];
                }
                ...
                footer.innerHTML = `
                    <div class="mt-4 font-pro" style="page-break-inside: avoid;">
                        ...
                        <!-- TOTALES - Premium Block -->
                        <div class="mb-4 rounded-2xl bg-white shadow-sm border border-slate-200 overflow-hidden flex relative" style="-webkit-print-color-adjust: exact; print-color-adjust: exact;">
                            <div class="w-1.5" style="background: linear-gradient(to bottom, ${currentHotel === 'cumbria' ? '#0f172a, #334155' : '#16a34a, #14532d'}); -webkit-print-color-adjust: exact; print-color-adjust: exact;"></div>
                            <div class="flex-1 px-8 py-4 flex justify-between items-center bg-gradient-to-r from-slate-50/40 to-white">
                                <div>
                                    <span class="text-[8px] font-black uppercase tracking-[0.25em] text-slate-400 block mb-1">Total Presupuestado</span>
                                    <div class="flex items-baseline gap-1">
                                        <span class="tabular-nums tracking-tighter" style="font-size: 34px; font-weight: 900; color: #0f172a; -webkit-print-color-adjust: exact; print-color-adjust: exact; letter-spacing: -0.02em;">${formatNum(totalNetoPagar)}</span>
                                        <span class="font-bold text-slate-400" style="font-size: 14px;">€</span>
                                    </div>
                                </div>
                                ${totalPaid >= Math.abs(totalNetoPagar) ? `
                                <div class="text-right flex flex-col items-end gap-1.5 border-l border-slate-100 pl-8">
                                    <span class="px-3 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-[0.15em] flex items-center gap-1.5 shadow-sm" style="background-color: #ecfdf5; color: #059669; border: 1px solid #a7f3d0; -webkit-print-color-adjust: exact; print-color-adjust: exact;">
                                    ...
                                    <span class="text-[8px] font-bold tabular-nums text-slate-400 tracking-wide mt-1">0,00 € pendiente</span>
                                </div>
                                ` : `
                                <div class="text-right border-l border-slate-200/60 pl-8 flex flex-col justify-center">
                                    <span class="text-[7.5px] font-black uppercase tracking-[0.25em] block mb-1" style="color: ${currentHotel === 'cumbria' ? '#475569' : '#15803d'};">Pendiente a Pagar</span>
                                    <div class="flex items-baseline justify-end gap-1">
                                        <span class="tabular-nums tracking-tighter" style="font-size: 26px; font-weight: 900; color: ${currentHotel === 'cumbria' ? '#0f172a' : '#14532d'}; -webkit-print-color-adjust: exact; print-color-adjust: exact; letter-spacing: -0.02em;">${formatNum(totalPending)}</span>
                                        <span class="font-bold" style="font-size: 13px; color: ${currentHotel === 'cumbria' ? '#64748b' : '#22c55e'};">€</span>
                                    </div>
                                </div>
                                `}
                            </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                <div class="flex items-center gap-2 mb-2">
                                    <h5 class="text-[7.5px] font-black text-slate-400 uppercase tracking-[0.2em]">Plan de Garantía y Pagos</h5>
                                </div>
                    ...
                    return `
                                            <div class="rounded-xl relative overflow-hidden" style="border: 1.5px solid ${isPaid ? '#a7f3d0' : '#e2e8f0'}; ${isPaid ? 'background-color: #ecfdf5;' : 'background-color: #ffffff;'} -webkit-print-color-adjust: exact; print-color-adjust: exact;">
                                                ...
                                                <div class="p-2 relative z-10 flex flex-col h-full justify-between">
                                                    ...
                                                    <div class="flex justify-between items-end mt-1">
                                                        <div>
                                                            <p class="text-[9px] ${isPaid ? 'text-emerald-500' : 'text-rose-600'} font-black uppercase tracking-wider mb-1">${isPaid ? 'Cobrado' : 'Fecha Límite'}</p>
                                                            <p class="text-[12px] font-bold ${isPaid ? 'text-slate-600' : 'text-slate-800'}">${formatDateSafe(depDate)}</p>
                                                        </div>
                                                        <div class="text-right">
                                                            <p class="tabular-nums leading-none flex items-baseline gap-0.5" style="font-size: 13px; font-weight: 900; ${isPaid ? 'color: #047857;' : 'color: #1e293b;'} -webkit-print-color-adjust: exact; print-color-adjust: exact;">${formatNum(dep.amount)}<span style="font-size: 8px; opacity: 0.6;">€</span></p>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                <div class="pt-1 pb-2 w-full flex justify-center">
                                    <div class="flex items-center gap-1.5 opacity-80" style="-webkit-print-color-adjust: exact; print-color-adjust: exact; font-size: 9px; line-height: 1;">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" style="color: #e11d48;" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg>
                                        <span class="text-slate-500 font-medium tracking-wide whitespace-nowrap"><span class="font-bold text-slate-600">TÉRMINOS DE PAGO:</span> Todos los pagos anticipados a cuenta son <strong class="font-bold text-slate-700">No Reembolsables</strong>.</span>
                                    </div>
                                </div>
            ...
            // Update static QR payment block
            ...
            const qrInner = document.getElementById('payment-qr-inner');
            if (qrBlock && payUrl && groupPlan.some(p => p.status !== "Cobrado")) {
                ...
                if (qrInner) {
                    const grad = currentHotel === 'cumbria' ? 'linear-gradient(135deg,#0f172a,#1e293b)' : 'linear-gradient(135deg,#064e3b,#065f46)';
                    qrInner.style.background = grad;
                }
            } else if (qrBlock) {
                        ...
                        <table style="width: 100%; border-collapse: collapse; font-size: 8px; text-transform: uppercase;">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td style="text-align: right; padding-top: 4px;">${formatNum(base10)} €</td>
                                    <td style="text-align: right; padding-top: 4px;">${formatNum(iva10)} €</td>
                                </tr>` : ''}
                                ${base21 > 0 ? `
                                <tr>
                                    <td style="padding-top: 2px;">GRAL. (21%)</td>
                                    <td style="text-align: right; padding-top: 2px;">${formatNum(base21)} €</td>
                                    <td style="text-align: right; padding-top: 2px;">${formatNum(iva21)} €</td>
                                </tr>` : ''}
                                <tr style="border-top: 1px solid rgba(255,255,255,0.3); font-weight: bold;">
                                    <td style="padding-top: 6px;">TOTAL NETO</td>
                                    <td colspan="2" style="text-align: right; padding-top: 6px; font-size: 10px;">${formatNum(base10 + base21)} €</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>`;
                ...
                footer.innerHTML = `
                    <div style="margin-top: 20px; width: 100%; display: flex; justify-content: flex-end;">
                        <div style="width: 320px; font-family: 'Montserrat', sans-serif;">
                            <!-- Total Section Premium Block -->
                            <div class="rounded-xl bg-white shadow-sm border border-slate-200 overflow-hidden flex relative w-full mb-2" style="-webkit-print-color-adjust: exact; print-color-adjust: exact;">
                                <div class="w-1.5" style="background: linear-gradient(to bottom, ${currentHotel === 'cumbria' ? '#0f172a, #334155' : '#16a34a, #14532d'}); -webkit-print-color-adjust: exact; print-color-adjust: exact;"></div>
                                <div class="flex-1 px-4 py-3 flex justify-between items-center bg-gradient-to-r from-slate-50/50 to-white">
                                    <span class="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400">Total IVA Incl.</span>
                                    <div class="flex items-baseline gap-1">
                                        <span class="tabular-nums tracking-tighter" style="font-size: 22px; font-weight: 900; color: #0f172a; -webkit-print-color-adjust: exact; print-color-adjust: exact; letter-spacing: -0.02em;">${formatNum(total - totalComision)}</span>
                                        <span class="font-bold text-slate-400" style="font-size: 11px;">€</span>
                                    </div>
                                </div>
                            </div>
                            
                            <!-- Discrete IVA Breakdown Section -->
                            <div style="margin-top: 10px; border-top: 1px solid #e2e8f0; padding: 6px 0;">
                                <table style="width: 100%; border-collapse: collapse; font-size: 9px; color: #334155; font-family: 'Montserrat', sans-serif; text-transform: uppercase;">
                                    <tr>
                                        <th style="text-align: left; font-weight: 800; color: #64748b; padding-bottom: 4px;">Base Imponible</th>
                                        <th style="text-align: center; font-weight: 800; color: #64748b; padding-bottom: 4px;">IVA</th>
                                        <th style="text-align: right; font-weight: 800; color: #64748b; padding-bottom: 4px;">Cuota</th>
                                    </tr>
                                    ${base10 > 0 ? `
                                    <tr>
                                        <td style="padding: 3px 0; font-weight: 600;">${formatNum(base10)} €</td>
                                        <td style="text-align: center; padding: 3px 0; font-weight: 600;">10%</td>
                                        <td style="text-align: right; padding: 3px 0; font-weight: 600;">${formatNum(iva10)} €</td>
                                    </tr>` : ''}
                                    ${base21 > 0 ? `
                                    <tr>
                                        <td style="padding: 3px 0; font-weight: 600;">${formatNum(base21)} €</td>
                                        <td style="text-align: center; padding: 3px 0; font-weight: 600;">21%</td>
                                        <td style="text-align: right; padding: 3px 0; font-weight: 600;">${formatNum(iva21)} €</td>
                                    </tr>` : ''}
                                </table>
                            </div>
                            ...
                            <div id="bank-details-wrapper" style="margin-top: 10px; text-align: right; border-top: 1px solid #eee; pt-4; position: relative;" class="group">
                                 ${(showBankDetails && !(groupData["Enlace_TPV"] || h.paymentGateway)) ? `
                                     <button class="no-print absolute top-0 -left-6 text-red-300 hover:text-red-500 text-xs" 
                                             onclick="showBankDetails=false;renderItems();;" title="Eliminar datos bancarios">✕</button>
                                 ...
                                 ${(groupData["Enlace_TPV"] || h.paymentGateway) ? `
                                    <p style="font-size: 8px; color: #059669; font-weight: 800; text-transform: uppercase; margin-top: 4px; display: flex; align-items: center; justify-content: flex-end; gap: 4px;">
                                        ...
                                        <a href="${(()=>{ const l = groupData['Enlace_TPV'] || h.paymentGateway || ''; return (l && !l.startsWith('http')) ? 'https://'+l : l; })()}" target="_blank" style="color: #059669; text-decoration: none; border-bottom: 1px solid #d1fae5; position: relative; z-index: 50;">Pago Online con Tarjeta</a>
                                    </p>
                                 ` : (!showBankDetails ? `
                                ...
                                <div id="legal-notice-wrapper" style="position: relative; border-top: 0.5px solid #f3f4f6; pt-2">
                                    ${showLegalNotice ? `
                                        <button class="no-print absolute top-2 -left-6 text-red-100 hover:text-red-300 text-[10px]" 
                                                onclick="showLegalNotice=false;renderItems();" title="Eliminar aviso legal">✕</button>
                                        <p style="font-size: 7px; color: #9ca3af; font-style: italic; font-weight: 400; line-height: 1.3;">
                                            * Este documento es una liquidación provisional (Factura Proforma).<br>
                                            No tiene validez como factura ordinaria hasta su emisión definitiva.
                                        </p>
                                    ` : `
                                        <button class="no-print text-gray-200 hover:text-gray-400 font-medium uppercase tracking-tighter" 
        ...
        async function syncWithFicha() {
            ...
            // 2. Fechas: siempre tomamos de group (formato ISO de Firestore)
            // NO leer del DOM para evitar sobreescribir con fechas en formato de pantalla (DD-MM-YYYY)
            const entrada = toInputDate(group["Entrada"]) || group["Entrada"];
            const salida = toInputDate(group["Salida"]) || group["Salida"];

            // 4. Actualizar Objeto Grupo
            // IMPORTANTE: NO modificamos Hotel_Asignado - ese campo se gestiona solo desde la ficha.
            // Si lo sobreescribimos aquí, grupos de Cumbria se guardarían como Guadiana al salir.
            const customFields = {};
            const editableIds = [
            ...
            const updatedData = {
                // IMPORTANTE: NO sobreescribir Pax. e Importe(*) -- son datos del PMS/importación.
                // Si los sobreescribimos con valores de la proforma, cada visita a la proforma
                // genera cambios falsos en la validación de importación.
                // Los guardamos como campos proforma-only para referencia interna:
                "Com_ProformaTotal": totalRevenue,
                "Com_ProformaPax": customFields['reserva-personas'] || group["Pax."],
                "Entrada": entrada,
                "Salida": salida,
                // Hotel_Asignado se omite intencionalmente — no debe modificarse desde la proforma
            ...
            if (finalGroup.Reserva && typeof db !== 'undefined') {
                try {
                    // IMPORTANT: Same normalization as normalizeId() in Gestión de Grupos.html
                    // to ensure Firestore doc ID is always consistent
                    const finalGroupReservaId = String(finalGroup.Reserva || "").trim().replace(/\.0$/, "").replace(/[\/\\]/g, "-");
                    await db.collection("groups").doc(finalGroupReservaId).set(updatedData, { merge: true });
                    // log
                } catch (e) {
                    console.error("❌ Error persistiendo en Firestore:", e);
                }
            }
        }
        ...
        // Helper: actualiza el enlace de pasarela con la URL real
        function updatePaymentLink() {
            const savedGroup = JSON.parse(localStorage.getItem('selectedGroup') || '{}');
            const h = HOTELS[currentHotel] || {};
            const l = savedGroup['Enlace_TPV'] || h.paymentGateway || '';
            const url = (l && !l.startsWith('http')) ? 'https://' + l : l;
            const link = document.getElementById('payment-qr-link');
            const img = document.getElementById('payment-qr-img');
            const rawUrl = document.getElementById('payment-raw-url');
            if (link && url) {
                link.href = url;
            ...
            if (img && url) {
                img.src = 'https://api.qrserver.com/v1/create-qr-code/?size=100x100&color=064e3b&bgcolor=ffffff&data=' + encodeURIComponent(url);
            }
        }

        // Botón Imprimir: actualizar enlace ANTES de llamar a window.print()
        function printDoc() {
            updatePaymentLink();
        ...
        window.onbeforeprint = () => {
            updatePaymentLink();
            const notas = document.getElementById('propuesta-notas');
            const placeholder = "Escriba aquí cualquier observación adicional";
            if (notas && (notas.innerText.includes(placeholder) || !notas.innerText.trim())) {
                notas.closest('.mt-8.pt-8').classList.add('no-print');
            }
        };
        window.onafterprint = () => {
            const notas = document.getElementById('propuesta-notas');
            if (notas) {
                notas.closest('.mt-8.pt-8').classList.remove('no-print');
            }
        };
