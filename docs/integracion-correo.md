...
- El botón Contestar descarga una respuesta al último mensaje: el usuario abre `respuesta.nexusreply` con el enlace local de `outlook/` para preparar el borrador en Outlook clásico. La apertura directa por protocolo queda como opción secundaria porque algunos navegadores no reconocen el enlace aun estando registrado en Windows. Outlook conserva la firma automática para mensajes nuevos y el enlace inserta debajo el texto original. Requiere instalación por equipo, firma configurada y revisión del remitente. No confirma ni registra un envío. Ver `outlook/LEEME.md`.
...
`npm run build:peticiones` compila JSX y CSS local de esta pantalla. Peticiones usa React y Firebase App/Auth/Functions desde `js/vendor/`, sin Tailwind CDN ni inicialización de Firestore. Tiene favicon relativo al proyecto. No se desactiva la prevención de seguimiento del navegador; los servicios remotos de autenticación siguen sujetos a las políticas del equipo.
